...
import { ShieldCheck, ArrowLeft, ArrowUpRight, Lock, CheckCircle2, Calendar } from 'lucide-react';
import { useEditorStore } from '../../store/editorStore';
import { PRIVACY_POLICY_DATA } from '../../data/legalData';
  ...
  return (
    <div className="w-full h-full overflow-y-auto bg-surface-950 text-surface-100 custom-scrollbar select-none">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-10 pb-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <div className="p-2 bg-success-500/20 border border-success-500/30 rounded-xl text-success-400">
              <ShieldCheck className="w-5 h-5" />
            </div>
            <span className="text-xs font-bold tracking-wider text-success-400 uppercase">
              Data & Privacy Notice
            </span>
          </div>
          ...
          <p className="text-sm text-surface-400 max-w-2xl">{PRIVACY_POLICY_DATA.subtitle}</p>
          <div className="flex flex-wrap items-center gap-4 pt-1 text-xs text-surface-500">
            <div className="flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5 text-surface-400" />
              <span>Last updated: {PRIVACY_POLICY_DATA.lastUpdated}</span>
            </div>
            <span>•</span>
            <span className="font-mono text-surface-400">
              Version {PRIVACY_POLICY_DATA.version}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 mb-8">
        <div className="p-5 rounded-2xl bg-gradient-to-tr from-success-950/40 via-surface-900 to-surface-900 border border-success-500/20 flex items-start gap-3.5 shadow-xl">
          <Lock className="w-5 h-5 text-success-400 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <h3 className="text-xs font-bold uppercase tracking-wider text-success-300">
              Privacy Highlights
            </h3>
            <p className="text-xs sm:text-sm text-surface-300 leading-relaxed">
              {PRIVACY_POLICY_DATA.summary}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="p-6 rounded-3xl bg-surface-900 border border-surface-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-center sm:text-left space-y-1">
            <span className="text-xs font-semibold text-surface-400">
              Looking for our terms of usage?
            </span>
            <p className="text-xs text-surface-300">
              Read our guidelines on copyright, licenses, and exported assets.
            </p>
          </div>
          <button
